...
public:
    ...
    if(auto* n = dynamic_cast<const BinaryNode*>(node)){
        ...
        bool any_float = std::holds_alternative<double>(left) || std::holds_alternative<double>(right);

        auto to_ll = [](Value v) -> long long {
            return std::holds_alternative<long long>(v) ? std::get<long long>(v) : (long long)std::get<double>(v);
        };
        auto to_d = [](Value v) -> double {
            return std::holds_alternative<double>(v) ? std::get<double>(v) : (double)std::get<long long>(v);
        };

        switch(n->op){
            case '+': return any_float ? Value(to_d(left) + to_d(right)) : Value(to_ll(left) + to_ll(right));
            case '-': return any_float ? Value(to_d(left) - to_d(right)) : Value(to_ll(left) - to_ll(right));
            case '*': return any_float ? Value(to_d(left) * to_d(right)) : Value(to_ll(left) * to_ll(right));
            case '/':
            ...
            case '%':
                ...
                return any_float ? Value(std::fmod(to_d(left), to_d(right))) : Value(to_ll(left) % to_ll(right));
    ...
    if(auto* n = dynamic_cast<const FunctionNode*>(node)){
        ...
        double arg = std::holds_alternative<double>(arg_val) ? std::get<double>(arg_val) : (double)std::get<long long>(arg_val);
